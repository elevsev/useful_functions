def barchart_of_categorical_features(data, cat_column, cat_column_2=None, no_results=20):
    ...
    if cat_column_2==None:
        ...
        pivot_data = pd.crosstab(index=data[cat_column], columns=cat_column)
        pivot_data['sum'] = pivot_data.sum(axis=1)
        pivot_data = pivot_data.sort_values(by='sum',ascending=False)
        pivot_data = pivot_data.drop(columns='sum')#
        pivot_data = pivot_data[:20]
        print(pivot_data)
        pivot_data.plot.barh(stacked=True, figsize=(20,10))
#         sns.catplot(y=cat_column, kind="count", data=pivot_data, hue=cat_column_2, order=plot_order, aspect=3)

    else:
        print("Bar Plot of top "+str(no_results)+' ' +cat_column+" with respect to "+cat_column_2+"\n")
        pivot_data = pd.crosstab(index=data[cat_column], columns=data[cat_column_2])
        pivot_data['sum'] = pivot_data.sum(axis=1)
        pivot_data = pivot_data.sort_values(by='sum',ascending=False)
        pivot_data = pivot_data.drop(columns='sum')#
        pivot_data = pivot_data[:no_results]
        pivot_data[:no_results].plot.barh(stacked=True, figsize=(30,15))
    plt.show()
    
    
def get_summary_of_categorical_features(data, cat_column, cat_column_2=None):
